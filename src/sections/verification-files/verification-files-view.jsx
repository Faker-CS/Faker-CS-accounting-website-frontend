...
export function VerificationFilesView() {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/companies/${userData.company_id}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => setCompanyName(res.data.company_name || ''))
      .catch(() => setCompanyName(''));
    // Fetch files for this company
    axios.get(`/api/companies/${userData.company_id}/files`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleDelete = async (file) => {
    try {
      ...
      const token = localStorage.getItem('jwt_access_token');
      await axios.delete(`/api/companies/${userData.company_id}/files`, {
        headers: { Authorization: `Bearer ${token}` },
        data: { filePath }
      });
      toast.success('File deleted successfully');
      // Refresh files
      const filesRes = await axios.get(`/api/companies/${userData.company_id}/files`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <DashboardContent>
      <Stack spacing={3}>
        <Typography variant="h4">Verification Files</Typography>
