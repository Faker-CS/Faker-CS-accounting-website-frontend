...
export default function SaPageView({ data, loading }) {
  ...
  useEffect(() => {
    const fetchServiceStatus = async () => {
      try {
        const response = await axios.get(`http://35.171.211.165:8000/api/status/5`, {
          headers: {
  ...
  return (
    <DashboardContent>
      ...
      {loading ? (
        ...
        <FileManagerView files={data} setServiceStatus={setServiceStatus} serviceId={6} />
      )}
    </DashboardContent>
  );
} 